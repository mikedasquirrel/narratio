"""
Discover Movie Archetypes

Applies all 7 discovery transformers to movie dataset to learn:
- Ξ (Golden Narratio) for movies
- Optimal α for entertainment
- Universal story arc patterns
- Cross-domain transferable patterns

This builds the ENTERTAINMENT ARCHETYPE for transfer to music, TV, books.
...
class MovieArchetypeDiscoverer:
    """Discovers movie narrative archetypes."""
    ...
    def load_movies(self):
        """Load movie dataset."""
        print("="*80)
        print("STEP 1: LOADING MOVIE DATA")
        print("="*80)
        ...
        print(f"Processing up to {self.max_samples} movies...")
        
        for i, movie in enumerate(movies[:self.max_samples]):
            if i % 100 == 0:
                print(f"  Processed {i}/{min(len(movies), self.max_samples)} movies...")
            
            if 'plot_summary' in movie and movie['plot_summary'] and len(movie['plot_summary']) > 50:
                texts.append(movie['plot_summary'])
                
                # Outcome: commercial success (box office)
                revenue = movie.get('box_office_revenue', 0)
                # Binary: success if > $10M
        ...
        print(f"\n✓ Loaded {len(texts)} movies with plot summaries")
        print(f"  Success rate (>${10}M box office): {success_rate:.1%}")
    ...
    def extract_genome(self):
        """Extract genome features from movies."""
        print("\n" + "="*80)
        ...
        transformers = [
            ...
            # Removed slow transformers for performance:
            # ('Emotional Resonance', EmotionalResonanceTransformer()),
            # ('Conflict Tension', ConflictTensionTransformer())
        ]
    ...
    def discover_structural_patterns(self):
        ...
        print("  Extracting arc shapes, tension geometry, archetypal structures...")
        
        transformer = UniversalStructuralPatternTransformer(
            n_sequence_points=50,
            use_fft=True,
        ...
        tension = features[:, 12]
        hero_journey_fit = features[:, -5]
        
        # Correlation with success
        corr_slope = pearsonr(arc_slope, self.outcomes)[0]
        corr_tension = pearsonr(tension, self.outcomes)[0]
        corr_hero = pearsonr(hero_journey_fit, self.outcomes)[0]
        
        print(f"\n  Discovered Patterns:")
        print(f"    Arc slope → success: r = {corr_slope:+.3f}")
        print(f"    Tension → success: r = {corr_tension:+.3f}")
        print(f"    Hero's journey fit → success: r = {corr_hero:+.3f}")
        ...
        self.results['structural_patterns'] = {
            ...
            'hero_journey_correlation': corr_hero
        }
        
        if abs(corr_hero) > 0.2:
            self.results['discovered_patterns'].append(
                f"Hero's journey arc predicts movie success: r={corr_hero:.3f}"
            )
    ...
    def discover_embedding_space(self):
        ...
        print("  Projecting movies into universal narrative space...")
        ...
        embedding_data = [
            {
                ...
                'domain': 'movies',
                'text': self.texts[i][:200]  # Truncate for memory
            }
        ...
        print("  Fitting embedder (this may take a moment)...")
        ...
        for cluster in range(transformer.metadata['n_clusters']):
            ...
            print(f"    Cluster {cluster}: {count:4} movies ({pct:5.1f}%)")
    ...
    def discover_golden_narratio(self):
        ...
        print("  Learning what WINNING MOVIES look like...")
        ...
        print("  Clustering winners to find Ξ...")
        features = transformer.fit_transform(embedding_data, y=self.outcomes)
        
        print(f"  ✓ Discovered Ξ (Golden Narratio) for movies")
        ...
        if abs(corr_xi) > 0.3:
            print(f"    🎯 STRONG PATTERN: Movies closer to Ξ perform better!")
        ...
        # Save archetype
        self.movie_xi = transformer.xi_vector_
        self.movie_alpha = optimal_alpha
    ...
    def discover_novelty_patterns(self):
        ...
        print("  Detecting unusual/novel movie narratives...")
        ...
        if high_novelty_success > low_novelty_success + 0.1:
            discovery = "Novelty HELPS movie success (originality rewarded)"
        elif high_novelty_success < low_novelty_success - 0.1:
            discovery = "Novelty HURTS movie success (familiarity preferred)"
        else:
    ...
    def save_archetype_library(self):
        ...
        print("\n" + "="*80)
        print("STEP 8: SAVING ARCHETYPE LIBRARY")
        print("="*80)
        ...
        archetype_library = {
            ...
            'timestamp': datetime.now().isoformat(),
            'xi_vector': self.movie_xi.tolist(),
            'optimal_alpha': float(self.movie_alpha),
        ...
        # Save to file
        output_path = Path('archetypes/movies_archetype_library.pkl')
        output_path.parent.mkdir(exist_ok=True)
        
        with open(output_path, 'wb') as f:
            pickle.dump(archetype_library, f)
        
        print(f"✓ Saved movie archetype library to: {output_path}")
        ...
        # Save full results
        results_path = Path('results/movie_discovery_results.json')
    ...
    def generate_report(self):
        ...
        print("\n" + "="*80)
        print("DISCOVERY REPORT: MOVIES")
        print("="*80)
        
        print(f"\nDataset: {self.results['n_samples']} movies")
        ...
        print(f"\n1. GOLDEN NARRATIO (Ξ) FOR MOVIES:")
        ...
        if 'structural_patterns' in self.results:
            ...
            print(f"   - Arc slope correlation: r = {sp['arc_slope_correlation']:+.3f}")
            print(f"   - Tension correlation: r = {sp['tension_correlation']:+.3f}")
            print(f"   - Hero's journey: r = {sp['hero_journey_correlation']:+.3f}")
        ...
        print("="*80)
        print("\nMovie archetype can now transfer to:")
        print("  - Music (π=0.702, entertainment)")
        print("  - Oscars (π=0.75, prestige entertainment)")
        print("  - Books/novels (narrative structure)")
        print("  - TV shows (episodic narratives)")
        print("  - Video games (interactive narratives)")
        
        print(f"\n✓ Analysis complete. Archetype library saved.")
...
def main():
    ...
    discoverer = MovieArchetypeDiscoverer(max_samples=200)
    
    discoverer.load_movies()
    ...
    print("\n" + "="*80)
    print("MOVIE ARCHETYPE DISCOVERY COMPLETE")
    print("="*80)
    print("\nNext steps:")
    print("  1. Review: results/movie_discovery_results.json")
    print("  2. Use archetype: archetypes/movies_archetype_library.pkl")
    print("  3. Transfer to: Music, Oscars, Books")
    print("="*80)
