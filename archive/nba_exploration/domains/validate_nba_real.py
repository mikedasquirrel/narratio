"""
NBA Full Validation with Proper Outcome Extraction

Extracts home game outcomes correctly and runs complete validation.
"""
...
def main():
    ...
    # 2. Extract home games only (to avoid duplicates)
    print("Extracting home game outcomes...")
    home_indices = []
    home_outcomes = []
    
    for i, game in enumerate(games):
        if game.get('home_game', False):
            home_indices.append(i)
            home_outcomes.append(1 if game['won'] else 0)
    
    X = X_all[home_indices]
    y = np.array(home_outcomes)
    
    print(f"✓ Extracted {len(X)} home games")
    ...
    # 7. Print summary
    print(f"\n{'='*80}")
    print("NBA BETTING VALIDATION SUMMARY")
    print(f"{'='*80}\n")
