#!/usr/bin/env python3
"""
Collect ESPN NHL scoreboards across multiple seasons for downstream modeling.

We mirror the NCAAB ingestion flow: walk the ESPN scoreboard API day-by-day,
dedupe events, bucket them by season label, and persist both the raw payload and
an at-a-glance summary so later jobs can verify coverage before merging odds.
"""
...
DATA_ROOT = PROJECT_ROOT / "data" / "nhl_season"
...
def fetch_day(session: requests.Session, target_date: date) -> Optional[Dict[str, object]]:
    ...
    for attempt in range(3):
        try:
            ...
            print(f"[WARN] NHL fetch {target_date} failed ({exc}); retrying in {wait:.1f}s")
            time.sleep(wait)
    print(f"[ERROR] NHL fetch {target_date} exhausted retries")
    return None
...
def collect_scoreboards() -> Dict[str, List[Dict[str, object]]]:
    ...
    for current_date in daterange(START_DATE, END_DATE):
        ...
        if len(events) >= REQUEST_LIMIT:
            print(f"[WARN] NHL {current_date} returned {len(events)} events (limit={REQUEST_LIMIT})")

        time.sleep(REQUEST_SLEEP)

    if not seasons:
        raise RuntimeError("No NHL scoreboard data collected.")
    ...
    print(f"Collected {len(seen_ids)} NHL games across {len(sorted_seasons)} season buckets.")
